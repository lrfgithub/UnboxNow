...
RESTful APIs are employed for seamless interaction with the frontend, while Nginx servers act as reverse proxies for all micro-services.
...
When a user initiates a "forgot-password" flow, the user micro-service issues a reset token, stores it in Redis and returns it as as a header in the HTTP response. The user can then reset the password by entering the specialized URL that contains the reset token. Reset tokens are validated and compared with stored ones during usage.